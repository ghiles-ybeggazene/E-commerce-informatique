			...
			if (preg_match('#^[1-9][0-9]*$#', $quantite)) 
			{
				if (preg_match('#^[1-9][0-9]*$#', $prix)) 
				{
					...
					if (count($ligne['ref'])==0) 
					{
						...
						$ins -> execute(array($ref,$ct1,$prix,$information,$image,$quantite,'oui'));
						$_SESSION['ajt_promo_reussit']='';
						header('location:gestion_promotion.php');
					}
					else
					{
						$_SESSION['erreur_promo_existe']='';
						header('location:ajouter_promotion.php');
					}	
				}
				else
				{
					$_SESSION['erreur_promo_prix']='';
					header('location:ajouter_promotion.php');
				}
			}
			else
			{
				$_SESSION['erreur_promo_qte']='';
				header('location:ajouter_promotion.php');
			}
