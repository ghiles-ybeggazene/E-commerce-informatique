...
<meta charset="utf-8">
	...
	.c{color:red;}
		...
		<div class="ta9">
			...
			<div>
				<form method="post" id="formM" action="modifier_produit.php">
					...
					<table class="table table-bordered table-hover">
						...
						<tr>
							<td>IMAGE</td>
							<td><img src="" height="150px;" width="150px;"><br><input type="text" name="img" value=""></td>
						</tr>
					</table>
...
<div class="text-center depl"><a href="ajouter_produit.php"><button class="btn btn-default btn-primary"> AJOUTER PRODUIT </button></a></div><br><br><br>
    <?php

	$req = $bdd -> query("SELECT categorie.*, classe.*, produit.* FROM (classe INNER JOIN categorie ON classe.id_classe = categorie.id_classe) INNER JOIN produit ON categorie.id_categorie = produit.id_categorie where promotion='non'");?>
	...
	<div class="row  text-center " style="margin-bottom:20px"> 
		<div class="panel panel-default">
			...
			<div class="panel-body"style="background-color:black;">
				...
				<form class="form-horizontal" style="margin-top:30px" method="post" action="gestion_produit.php">
...
if(isset($_POST['consult_rech']))
{
	if(isset($_POST['rech']) && !empty($_POST['rech']) && isset($_POST['critere']) && !empty($_POST['critere']))
	{
		...
		$req_rech = $bdd -> prepare('SELECT categorie.*, classe.*, produit.* FROM (classe INNER JOIN categorie ON classe.id_classe = categorie.id_classe) INNER JOIN produit ON categorie.id_categorie = produit.id_categorie WHERE '.$radio_rech.'=? and promotion="non"');
		...
		<tbody>
			<tr>
				...
				<td ><button class="btn btn-default btn-warning depl" onclick="modifier('<?php echo $res["ref_produit"]; ?>')"> MODIFIER PRODUIT </button></td>
				<td ><button class="btn btn-default btn-danger depl" onclick="supprimer('<?php echo $res["ref_produit"]; ?>')"> SUPPRIMER PRODUIT </button></td>
			</tr>
...
?>
	...
	<table class="table table-bordered table-compressed text-center table-hover">
		...
		<?php while($req1 = $req -> fetch())
		{
			...
			<tr>
				...
				<td ><button class="btn btn-default btn-warning depl" onclick="modifier('<?php echo $req1["ref_produit"]; ?>')"> MODIFIER PRODUIT </button></td>
				<td ><button class="btn btn-default btn-danger depl" onclick="supprimer('<?php echo $req1["ref_produit"]; ?>')"> SUPPRIMER PRODUIT </button></td>
			</tr>
			
			<?php
		}
	}
	if (isset($_SESSION['supp_prod_reussi'])) 
	{
		echo('<script>alert("suppression reussite!!")</script>');
		unset($_SESSION['supp_prod_reussi']);
	}

	if (isset($_SESSION['ajt_prod_reussit'])) 
	{
		echo("<script>alert('le produit est ajouté!!!');</script>");
		unset($_SESSION['ajt_prod_reussit']);
	}

	if(isset($_SESSION['modif_prod_reussi']))
	{
		echo('<script>alert("modification reussite");</script>');
		unset($_SESSION['modif_prod_reussi']);
	}

	if(isset($_SESSION['err_modif_prod_prix']))
	{
		echo('<script>alert("erreur !!! le prix est une valeur numerique");</script>');
		unset($_SESSION['err_modif_prod_prix']);
	}

	if(isset($_SESSION['err_modif_prod_qte']))
	{
		echo('<script>alert("erreur !!! la quantite est une valeur numerique")</script>');
		unset($_SESSION['err_modif_prod_qte']);
	}

	if(isset($_SESSION['err_modif_prod_gle']))
	{
		echo('<script>alert("erreur !!! vous n\'avez pas ecrit tous les champs");</script>');
		unset($_SESSION['err_modif_prod_gle']);
	}
}
		?>								
		</tbody>
	...
	<script>
		...
		{
			var sur = confirm("etes vous sur de supprimer ce produit ???");
			if(sur)
				{
					var formulaire = document.getElementById("form");
					formulaire.setAttribute("action","suppression_produit.php");
		...
		sc.setAttribute("src","info_produit.php?idu="+reference);
		document.body.appendChild(sc);
		}

		function modifier(reference){
			var sc = document.createElement("script");
			sc.setAttribute("src","info_produit.php?idu="+reference+"&mod=");
			document.body.appendChild(sc);
		}
