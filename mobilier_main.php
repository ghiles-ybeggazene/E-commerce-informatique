...
FROM (classe INNER JOIN categorie ON classe.id_classe = categorie.id_classe) INNER JOIN produit ON categorie.id_categorie = produit.id_categorie
 WHERE nom_classe="mobilier" AND promotion="non" ORDER BY  image and nom_categorie ');
...
while($donnee = $req -> fetch())
{
  ...
  <div class="col-xs-4 " >
    <div class="thumbnail"style="height:700px;">
      <img  class="img img-thumbnail im" src = "<?php echo $donnee['image']; ?>" style="height:400px;"/>
      <div class="caption">
       
      <p><?php 
      echo '<pre><h4 >'.$donnee['information'].'</h4></pre>';
    echo '<h2 style="font-weight:bold;"> Prix:<span style="color:red;">'. number_format( $donnee['prix'],'2').' DA</br></span> </h2> ';
